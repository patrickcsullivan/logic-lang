  ...
  LastProof ->
    case last of
      ...
      AnswerExtraction _ _ _ [] _ -> do
        ...
        mainLoop fileClauses last
      AnswerExtraction _ _ _ _ [] -> do
        outputStrLn "There are no additional proofs to show."
        mainLoop fileClauses last
  ...
  LastAnswer ->
    case last of
      ...
      AnswerExtraction vars _ _ ((answer, _) : _) _ -> do
        outputStrLn $ showAnswer vars answer
        mainLoop fileClauses last
  NextAnswer ->
    case last of
...
findAnswers fileClauses vars terminationCheck searchState shownAnswers unshownAnswers = case unshownAnswers of
  ...
  [] ->
    let (searchState', newAnswers) = resolutionLoop terminationCheck searchState
     in case newAnswers of
